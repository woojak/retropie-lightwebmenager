...
def settings():
    ...
    if request.method == 'POST':
        if 'save_credentials' in request.form:
            ...
            if npw: CONFIG['password'] = npw; PASSWORD = npw
            ...
            try:
                ...
                pass
            msg += "App settings updated. (Restart service for port changes.) "
  ...
  <div class="tab-content pt-3">
    ...
    <div id="app" class="tab-pane fade">
      <form method="post">
        ...
        <div class="mb-3">
          <label class="form-label">RPI Config File Location</label>
...
def edit_file(req_path):
    ...
    if not os.path.isfile(abs_p):
        flash("Selected item is not a file.")
    ...
    tmpl = """
<!doctype html>
<html lang="en"><head><meta charset="utf-8"><title>Edit File</title>
...
<body><div class="container py-4">
  ...
  <form method="post">
    ...
    <a class="btn btn-secondary" href="{{url_for('dir_listing',req_path=parent)}}">Cancel</a>
  </form>
</div></body></html>
"""
...
def edit_config():
    ...
    tmpl = """
<!doctype html>
<html lang="en"><head><meta charset="utf-8"><title>Edit RPI config.txt</title>
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.1/dist/css/bootstrap.min.css" rel="stylesheet"></head>
<body><div class="container py-4">
...
def create_folder(req_path):
    ...
    tgt = os.path.join(abs_p, secure_filename(name))
    try:
        os.makedirs(tgt)
...
def upload_file(req_path):
    abs_d = safe_path(req_path)
    if 'file' not in request.files:
        flash("No file selected.")
        return redirect(url_for('dir_listing', req_path=req_path))
    for f in request.files.getlist('file'):
        if not f.filename:
            flash("Unnamed file skipped.")
            continue
        fn = secure_filename(f.filename)
        dst = os.path.join(abs_d, fn)
        try:
            with open(dst, 'wb') as out:
                while True:
                    ...
                    if not chunk:
                        break
                    out.write(chunk)
            flash(f"Uploaded '{fn}' successfully.")
        except Exception as e:
...
def delete_folder(req_path):
    ...
    if not os.path.isdir(p):
        flash("Selected item is not a folder.")
    else:
        try:
            shutil.rmtree(p)
...
<!doctype html><html lang="en"><head>
  ...
  <style>.progress{height:40px}.progress-bar{color:black;font-size:1.2em;font-weight:bold}</style>
</head><body>
<div class="container py-4">
  ...
  {% with msgs=get_flashed_messages() %}
    {% if msgs %}
      {% for m in msgs %}<div class="alert alert-warning">{{m}}</div>{% endfor %}
    {% endif %}
  {% endwith %}
  <!-- Monitoring Section -->
  <div class="card mb-4">
    ...
    <div class="card-body">
      <div class="row mb-3">
        ...
        <div class="col-md-3"><strong>SSD Temp:</strong> <span id="ssd-temp">{{mon['ssd_temp']}}</span>
        ...
        <div class="col-md-3">
          ...
          <form method="get" class="d-flex align-items-center flex-wrap">
            <label class="me-2 mb-1"><small>Select NVMe Sensor:</small></label>
            <input type="hidden" name="req_path" value="{{req_path}}">
            <select name="ssd_sensor" class="form-select" style="max-width:200px" onchange="this.form.submit()">
      ...
      <div class="row mb-3">
        ...
        <div class="col-md-4">
          <div class="mb-1"><strong>Disk Usage:</strong> ({{mon['disk_used_human']}} used / {{mon['disk_total_human']}} total)</div>
  ...
  <div class="card mb-4">
    ...
    <div class="card-body">
      <form method="post" action="{{url_for('control')}}">
        <button name="action" value="reboot" class="btn btn-warning mb-2">Reboot Raspberry Pi</button>
        <button name="action" value="shutdown" class="btn btn-danger mb-2">Shutdown Raspberry Pi</button>
      </form>
    </div>
  </div>
    ...
    <div class="card-body">
      <div class="row">
        <div class="col-md-6">
          ...
          <div class="mt-2">
            <div id="uploadSpeed" class="text-center"></div>
            <div id="uploadStats" class="text-center"></div>
            <div id="uploadETA" class="text-center"></div>
          </div>
        </div>
        <div class="col-md-6">
    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        <table class="table table-striped table-hover">
          ...
          <tbody>
            {% if req_path %}
              <tr><td colspan="7"><a href="{{url_for('dir_listing',req_path=parent)}}" class="btn btn-sm btn-outline-dark"><i class="fas fa-level-up-alt"></i> [..]</a></td></tr>
            {% endif %}
            {% for f in files %}
              <tr>
...
<script>
// -- Update Monitoring at configured interval --
function updateMonitoring() {
  let sensorSelect = document.getElementById("ssd_sensor_select");
  let param = "";
  if (sensorSelect) {
    param = "?ssd_sensor=" + encodeURIComponent(sensorSelect.value);
  }
  fetch("/api/monitoring" + param)
    ...
    .catch(err => console.error("Error updating monitoring:", err));
}
...
document.getElementById("uploadForm").addEventListener("submit", function(e) {
  ...
  xhr.upload.addEventListener("progress", function(e) {
    ...
    if (dt > 0) {
      ...
      let transferred = fmt(e.loaded), remaining = fmt(e.total - e.loaded);
      document.getElementById("uploadStats").textContent = `Transferred: ${transferred} • Remaining: ${remaining}`;
